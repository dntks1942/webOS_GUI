...
var useScrollBase = function useScrollBase(props) {
  ...
  function onMouseDown(ev) {
    if ((0, _handle.forwardWithPrevent)('onMouseDown', ev, props)) {
      stop();
    }
  } // scrollMode 'native' [[
  ...
  function onFlick(ev) {
    if (scrollMode === 'translate') {
      mutableRef.current.flickTarget = mutableRef.current.animator.simulate(mutableRef.current.scrollLeft, mutableRef.current.scrollTop, direction !== 'vertical' ? getRtlX(-ev.velocityX) : 0, direction !== 'horizontal' ? -ev.velocityY : 0);
    } else if (scrollMode === 'native') {
      if (!mutableRef.current.isTouching) {
        mutableRef.current.flickTarget = mutableRef.current.animator.simulate(mutableRef.current.scrollLeft, mutableRef.current.scrollTop, direction !== 'vertical' ? getRtlX(-ev.velocityX) : 0, direction !== 'horizontal' ? -ev.velocityY : 0);
      } else if (mutableRef.current.overscrollEnabled && overscrollEffectOn && overscrollEffectOn.drag) {
        mutableRef.current.flickTarget = {
          targetX: mutableRef.current.scrollLeft + getRtlX(-ev.velocityX) * overscrollVelocityFactor,
          // 'horizontal' or 'both'
  ...
  */
  ...
  function checkAndApplyOverscrollEffect(orientation, edge, overscrollEffectType) {
    ...
    if (scrollMode === 'translate' && edge === 'before' && curPos <= 0 || edge === 'after' && curPos >= maxPos || scrollMode === 'native' && edge === 'before' && curPos <= 0 || edge === 'after' && curPos >= maxPos - 1) {
      // Already on the edge
  ...
  function forwardScrollEvent(overscrollEffectType, reachedEdgeInfo) {
    (0, _handle.forward)(overscrollEffectType, {
